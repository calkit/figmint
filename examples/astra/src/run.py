...
It is not part of fromwhere and it is not part of ASTRA. It is here so the example
can actually be run, and so the one interesting line is visible rather than
described:

    fromwhere run ... -- uv run python src/peak.py --fit polynomial ...

That command — decisions resolved, wrapper and all — is what lands in
`provenance.toml`. Which is the whole point of putting the two together: the record
