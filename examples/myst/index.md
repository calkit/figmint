---
title: "`fromwhere` tracks artifact provenance"
authors:
  - name: Example Author
---

# `fromwhere` tracks artifact provenance

A small document whose figure carries its own provenance. The panel below was
produced by a script fromwhere knows nothing about; what fromwhere knows is what the
...
from `provenance.toml` and the file's own credentials when the document builds.
Nothing on this page is typed by hand.

That last point is the one worth dwelling on. The schematic is AI-generated, and
fromwhere did not have to be told: the image carries C2PA credentials saying so,
...
fromwhere hashed the inputs, ran the command, hashed the output, and recorded all
of it. It did not read the script, and it does not need to.

fromwhere also recorded `scripts/plot_cp.py`, which was never passed with `-i`.
...
fromwhere status # the figure is now out of date
```
...
:::{warning}
`fromwhere status` reporting a figure as stale means the figure is stale. The fix
is to regenerate it — never to edit the hashes in `provenance.toml`. That file is
...
```sh
fromwhere drawio import figures/cp_curve.png figures/composite.drawio
fromwhere drawio import figures/turbine.png  figures/composite.drawio
# ...arrange it in draw.io, add the definition of Cp...
fromwhere drawio export figures/composite.drawio figures/composite.svg
```
...
Use the fromwhere command rather than draw.io's own **Insert → Image**. draw.io
...
`fromwhere drawio export` renders one, and does three things while it is there:
...
- **re-records the diagram**, because editing it in draw.io is the entire point
  ...
  behind fromwhere's back_ — would be worth nothing. Exporting is the deliberate
...
record itself, drawn: `provenance.toml` is already a DAG, so nothing here can
...
fromwhere never reads the script, so that edge is inferred from the data and the
...
`data/performance.csv` the line in `provenance.toml` is the _only_ provenance
there is.
...
Follow any figure back far enough and you reach a file fromwhere did not make.
Here that is `data/performance.csv` — measurements — and `figures/turbine.png`,
an image generated with an AI tool. `fromwhere run` cannot record those, because
fromwhere was not there.
...
check above them passes, which is what makes the gap easy to miss. `fromwhere
status` says so:

```
warning: nothing accounts for data/performance.csv
         declare it: `fromwhere declare <path> --mine [--with-ai ...]`,
         `--doi ...`, or `--git ...@rev`
```
...
fromwhere declare data/other.csv --doi 10.5281/zenodo.1234567
fromwhere declare data/shared.csv --git github.com/user/proj/data.csv@a1b2c3d
fromwhere declare data/dvc.csv --calkit calkit.io/user/proj/data.csv@a1b2c3d
```

`--mine` is an attestation — nothing verifies it, and fromwhere says so when you
...
fromwhere declare figures/composite.drawio --mine --with-ai 'Claude Opus 5'
```
...
```sh
fromwhere declare scripts/plot_cp.py --from-git-history
```
...
people who made it. Restating that in `provenance.toml` would create a second copy
...
an image silently discards it, which is the whole reason `fromwhere drawio
...
The declaration lives in `provenance.toml` beside the artifact's hash. There is no
...
already record. `fromwhere run` keeps the declared hash current as it goes, so
the record does not drift.

A hash fromwhere wrote _itself_ is a different matter. That is evidence, it is
...
once it leaves the repository. `provenance.toml` is the record while the figure is
...
Nothing in the build re-imports a panel. `fromwhere drawio import` is how a
figure gets _onto_ the canvas in the first place; after that the diagram knows
where each panel came from, and `fromwhere drawio export` re-embeds any that have
...
fromwhere records. `make site` runs the build through `fromwhere run`, so the
...
still consistent with everything behind it?" a question `fromwhere status` can
answer, which is the one a reader most wants answered and the one nothing else
checks.

Two kinds of circularity had to be kept out of the way. `provenance.toml` is
deliberately _not_ an input of the document, even though this plugin reads it
to draw the panels above — recording the file that `fromwhere run` itself writes
...
tally. `fromwhere status` checks it from outside, where the answer has settled.
...
`make serve` closes that gap by watching everything fromwhere records as an input
...
When they do, `fromwhere rebuild` puts it right: the record knows the command
...
`provenance.toml` is the only provenance there is, which is exactly why the
