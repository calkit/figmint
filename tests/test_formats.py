...
class TestDrawioSvg:
    """`.drawio.svg` — a rendered SVG carrying its own diagram XML.

    One file that is simultaneously the published picture and the editable
    source, which is what makes it worth publishing into MyST: the page embeds
    it directly, and figmint can still read every component's provenance out of
    the embedded diagram.
    """

    @pytest.fixture
    def rendered(self, tmp_path: Path) -> Path:
        (tmp_path / ".git").mkdir()
        shutil.copy(EXAMPLES / "figures" / "cp_curve.svg", tmp_path / "plot.svg")
        shutil.copy(
            EXAMPLES / "figures" / "cp_curve.svg.prov.yaml",
        ...
        source = tmp_path / "f.drawio"
        main(["place", str(tmp_path / "plot.svg"), "--into", str(source), "--create"])

        # Build the wrapped form the way draw.io does: the whole mxfile escaped
        # into a `content` attribute on the root <svg>.
        from xml.sax.saxutils import quoteattr

        mxfile = source.read_text()
        target = tmp_path / "f.drawio.svg"
        target.write_text(
            '<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" '
            f"content={quoteattr(mxfile)}><rect width=\"10\" height=\"10\"/></svg>"
        )
        return target

    def test_the_extension_dispatches_to_the_drawio_reader(self):
        assert formats.reader_for(Path("a.drawio.svg")) is DrawioDocument

    def test_components_are_read_from_the_embedded_diagram(self, rendered: Path):
        document = open_document(rendered)
        assert isinstance(document, DrawioDocument)
        assert len(document.components()) == 1
        assert document.components()[0].origin == "plot.svg"

    def test_it_is_marked_rendered(self, rendered: Path):
        assert open_document(rendered).rendered is True

    def test_check_works_on_it(self, rendered: Path):
        assert main(["check", str(rendered)]) == EXIT_OK

    def test_an_svg_without_an_embedded_diagram_is_rejected(self, tmp_path: Path):
        plain = tmp_path / "plain.drawio.svg"
        plain.write_text('<svg xmlns="http://www.w3.org/2000/svg"><rect/></svg>')
        with pytest.raises(DocumentError, match="no embedded diagram"):
            open_document(plain)

    def test_a_corrupt_embedded_diagram_is_rejected(self, tmp_path: Path):
        bad = tmp_path / "bad.drawio.svg"
        bad.write_text(
            '<svg xmlns="http://www.w3.org/2000/svg" content="not xml at all"/>'
        )
        with pytest.raises(DocumentError):
            open_document(bad)

    def test_overwriting_in_place_is_refused(self, rendered: Path):
        # The embedded XML is easy to update; the picture around it can only be
        # regenerated by draw.io. Writing here would leave a file whose image and
        # whose metadata disagree.
        document = open_document(rendered)
        with pytest.raises(DocumentError, match="refusing to overwrite"):
            document.save()

    def test_staleness_is_still_detectable(self, rendered: Path):
        (rendered.parent / "plot.svg").write_bytes(b"<svg>new</svg>")
        assert main(["reimport", str(rendered), "--check"]) == EXIT_STALE

    def test_reimport_in_place_is_refused(self, rendered: Path):
        (rendered.parent / "plot.svg").write_bytes(b"<svg>new</svg>")
        assert main(["reimport", str(rendered)]) == EXIT_ERROR

    def test_reimport_to_a_drawio_hands_off_cleanly(self, rendered: Path):
        # This is what makes a single published file workable: figmint writes the
        # updated diagram, draw.io re-renders the picture around it.
        (rendered.parent / "plot.svg").write_bytes(b"<svg>new</svg>")
        handoff = rendered.parent / "updated.drawio"

        assert main(["reimport", str(rendered), "-o", str(handoff)]) == EXIT_OK
        assert handoff.exists()

        refreshed = open_document(handoff)
        assert refreshed.components()[0].embedded == b"<svg>new</svg>"
        # A plain diagram, not a rendered one — draw.io's job from here.
        assert refreshed.rendered is False

    def test_adopt_on_a_rendered_file_is_refused(self, rendered: Path, capsys):
        assert main(["adopt", str(rendered)]) == EXIT_ERROR
